      ...
      <div className="max-w-4xl mx-auto">
        ...
        {activeTab === 'flashcards' && flashcards.length > 0 && (
          <div className="animate-in fade-in duration-300 flex flex-col items-center">
            <div className="w-full max-w-2xl text-center mb-4 text-slate-500 dark:text-slate-400 font-mono text-xs">
              Card {currentCardIndex + 1} of {flashcards.length}
            </div>
            
            <div 
              onClick={() => setIsFlipped(!isFlipped)}
              className={`w-full max-w-2xl min-h-[280px] bg-white dark:bg-[#172033] rounded-3xl border-2 cursor-pointer transition-all duration-300 flex items-center justify-center p-8 shadow-sm relative ${
                isFlipped 
              ...
              <div className="text-center">
                {!isFlipped ? (
                  <div className="space-y-4">
                    <span className="text-xs font-mono font-bold uppercase tracking-wider text-slate-400 dark:text-slate-500 block">Question</span>
                    <h2 className="text-xl sm:text-2xl font-bold text-slate-900 dark:text-slate-100 leading-snug">
                      {flashcards[currentCardIndex].q}
                    </h2>
                    <p className="text-xs text-slate-400 mt-6">Click card to reveal solution</p>
                  </div>
                ) : (
                  <div className="space-y-4 animate-in zoom-in-95 duration-200">
                    <span className="text-xs font-mono font-bold uppercase tracking-wider text-blue-600 dark:text-blue-400 block">Answer</span>
                    <h2 className="text-xl sm:text-2xl font-bold text-blue-900 dark:text-blue-100 leading-snug">
            ...
            <div className="flex items-center gap-4 mt-6">
              <button 
                onClick={prevCard}
                className="w-12 h-12 rounded-full bg-white dark:bg-[#172033] border border-slate-200 dark:border-slate-700 flex items-center justify-center hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors shadow-2xs cursor-pointer"
                title="Previous Card"
              >
              ...
              <button 
                onClick={() => setIsFlipped(!isFlipped)}
                className="px-6 py-3 rounded-xl font-bold text-xs bg-blue-600 hover:bg-blue-700 text-white shadow-xs transition-all flex items-center gap-2 cursor-pointer"
              >
