...
const normalizePhrase = (phrase: string | undefined | null, width: number): string => {
...
export const SplitFlapText: React.FC<SplitFlapTextProps> = ({
  ...
  useEffect(() => {
    ...
    const previousPhrase = currentTextRef.current;

    if (previousPhrase && previousPhrase !== targetPhrase && !prefersReducedMotion) {
  ...
  const componentStyle: CSSProperties = {
    ['--split-flap-tile-color' as any]: tileColor,
    ['--split-flap-text-color' as any]: textColor,
    ['--split-flap-radius' as any]: toCssUnit(tileRadius),
    ['--split-flap-gap' as any]: toCssUnit(gap),
    ['--split-flap-font-size' as any]: toCssUnit(fontSize),
    ['--split-flap-flip-duration' as any]: `${Math.max(0.04, Number(flipDuration) || 0.12)}s`,
    ...style
  };
  ...
  return (
    ...
    >
      {tiles.map((tile, index) => {
        const cur = tile?.current ?? ' ';
        const nxt = tile?.next ?? cur;
        const isFlipping = !!tile?.flipping;
        return (
          <span className="split-flap-text__tile" aria-hidden="true" key={`${index}-${tiles.length}`}>
            <span className="split-flap-text__half split-flap-text__half--top">
              <span className="split-flap-text__char">{(isFlipping ? nxt : cur) === ' ' ? '\u00A0' : (isFlipping ? nxt : cur)}</span>
            </span>
            <span className="split-flap-text__half split-flap-text__half--bottom">
              <span className="split-flap-text__char">{cur === ' ' ? '\u00A0' : cur}</span>
            </span>

            {isFlipping && (
              <>
                <span className="split-flap-text__flap split-flap-text__flap--front" key={`front-${index}-${tile?.tick || 0}`}>
                  <span className="split-flap-text__char">{cur === ' ' ? '\u00A0' : cur}</span>
                </span>
                <span className="split-flap-text__flap split-flap-text__flap--back" key={`back-${index}-${tile?.tick || 0}`}>
                  <span className="split-flap-text__char">{nxt === ' ' ? '\u00A0' : nxt}</span>
                </span>
              </>
            )}
          </span>
